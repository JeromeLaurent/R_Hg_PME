...
rm(list = ls () ) #nettoyage mémoire de R
...
# Chargement BDD comprenant données Criques Nouvelle-France, Chien et Trois Sauts

BDD <-read.csv("C:/Users/laurent/Desktop/Stage_Jerome_LAURENT/BDD/2014_04_11_subset_BDD.csv", sep=";", stringsAsFactors = FALSE)
...
# Les valeurs étant supposées être numériques sont numériques
...
scatter <- ggplot(BDD, aes(x = ls_mm, y = pds_g)) + 
  ...
  scale_y_continuous(limits = c(0, 100)) # 2 Hoplias aimara de presque 2 kg qui entraînent le tassement de la majorité du jeu de données

#marginal density of x - plot on top
plot_top <- ggplot(BDD, aes(ls_mm, fill=Regime_alter)) + 
  geom_density(alpha = .5) + 
  scale_x_continuous(limits = c(0, 200)) + # pour aperçu plus détaillé de la distrib de la majorité des poissons
  theme(legend.position = "none")

#### distribution approximative du poids
#ggplot(BDD, aes(x = Regime_alter, y = pds_g)) +
#  geom_boxplot() +
#  scale_y_continuous(limits = c(0, 200)) # la grande majorité est <200g

#marginal density of y - plot on the right
plot_right <- ggplot(BDD, aes(pds_g, fill=Regime_alter)) + 
  geom_density(alpha = .5) + 
  scale_x_continuous(limits = c(0, 50)) + # limites d'après divers essais. poissons très légers
  coord_flip() + 
  theme(legend.position = "none") 
...
# nuage de points alternatif avec zoom sur majorité de points
scatter <- ggplot(BDD, aes(x = ls_mm, y = pds_g)) + 
  geom_point(aes(color = Regime_alter, shape = Regime_principal)) + 
  theme(legend.position = c(1, 1), legend.justification = c(1, 1)) +
  scale_x_continuous(limits = c(0, 200)) + 
...
## Quelle est la répartition des régimes alimentaires parmi les poissons prélevés sur chaque groupe de station ?


# Réordonne les niveaux du facteur "Régime_principal" dans l'ordre décroissant de contamination au mercure
...
# Répartition des régimes à chaque groupe de stations pour BDD générale
p1 <- ggplot(BDD, aes(Groupe_station)) +
  geom_bar(aes(fill = Regime_alter), position = "fill")
# répartition des régimes à chaque groupe de stations (sans prendre en compte le nb d'individus)
p2 <- ggplot(BDD, aes(x = Groupe_station)) +
  geom_bar(aes(fill = Regime_alter))
# répartition des régimes à chaque groupe de stations (en prenant en compte le nb d'individus)
grid.arrange(p1, p2, ncol = 1, nrow = 2, widths = c(1, 1), heights = c(1, 1))

# Répartition des régimes à chaque groupe de stations pour subset BDD
...
# Répartition des régimes sur chaque station
p10 <- ggplot(BDD, aes(Code_Station)) +
  geom_bar(aes(fill = Regime_alter), position = "fill")
# répartition des régimes sur chaque station (sans prendre en compte le nb d'individus)
p20 <- ggplot(BDD, aes(x = Code_Station)) +
  geom_bar(aes(fill = Regime_alter))
# répartition des régimes sur chaque station (en prenant en compte le nb d'individus)
grid.arrange(p10, p20, ncol = 1, nrow = 2, widths = c(1, 1), heights = c(1, 1))

# La répartition des régimes est fondamentalement différente au niveau de la Crique Nouvelle France 6
# Peu d'individus ont été prélevés au niveau des criques Chien

# Répartition des genres sur chaque station
...
# Répartition des genres à chaque groupe de stations
...
## Quelle est la contamination moyenne en Hg parmi les poissons prélevés sur chaque groupe de station en fonction de leur régime alimentaire ?
...
## Quelle est la contamination moyenne en Hg parmi les poissons prélevés sur chaque station en fonction de leur régime alimentaire ?
...
## Quelle est la contamination moyenne en métaux sur chaque site ?
ggplot(dfmelt_mtx, aes(x = Site, y = value, color = variable)) +
  geom_boxplot()

## Quelle est la contamination moyenne en métaux sur chaque groupe de station ?
ggplot(dfmelt_mtx, aes(x = Groupe_station, y = value, color = variable)) +
  geom_boxplot()

## Quelle est la contamination moyenne en métaux sur chaque station ?
...
## Informations globales sur la contamination métallique
...
# ne fonctionne pas, je suppose qu'un ajustement manuel est nécessaire



# Exemple de représentation de distribution
ggplot(BDD, aes(Cu_ppm)) +
  geom_density(fill = "blue", colour = NA, alpha=.2) + # polygone bleu pour la densité
  geom_line(stat = "density") # contour du polygone en noir

# Pr chq metal, superposition densité théorique et histogramme de distribution
...
# tentative d'ajout de la répartition des régimes alimentaires au niveau de chq distribution. Non concluant mais non développé
...
# Pr chq métal, concentrations selon régime trophique
grid.arrange(p2, p3, p1, ncol = 3, nrow = 1, widths = c(6, 2, 2), heights = c(1, 1))

# Pr chq métal, concentrations selon genre. Beaucoup de données ; difficile de s'y retrouver
grid.arrange(p02, p03, p01, ncol = 3, nrow = 1, widths = c(6, 2, 2), heights = c(1, 1))

# Pr chq métal, concentration selon station
grid.arrange(p002, p003, p001, ncol = 3, nrow = 1, widths = c(6, 2, 2), heights = c(1, 1))

# Pr chq métal, concentration selon groupe de stations
...
# Pr chq genre, contamination métallique. Avec Hg 
grid.arrange(p0, p00, p, ncol = 1, nrow = 3, widths = c(1, 1), heights = c(1, 1))

# Pr chq genre, contamination métallique. Sans Hg ; un peu plus lisible
grid.arrange(p0, p00, p_0, ncol = 1, nrow = 3, widths = c(1, 1), heights = c(1, 1))

# Pr chq sp, contamination métallique. Sans Hg ; un peu plus lisible
grid.arrange(p88, p888, p_8, ncol = 1, nrow = 3, widths = c(1, 1), heights = c(1, 1))

# Pr chq sp, contamination métallique. Avec Hg
...
# Rq : les 3 distributions projetées en haut ne sont pas tout à fait correctes : il s'agit de l'ensemble des valeurs de d15N et non pas des valeurs ayant des [Hg] mesurées
...
# normes conta sous forme de lignes ajoutées à chaque graphe
...
# Pr chq site, différence selon omni/carni en fction du metal
...
# Pr chq groupe de site, différence selon régime en fction du metal
...
# Il y a une couleur qui ne devrait pas apparaître, comme si il y avait encore des valeurs manquantes...
...
fligner.test(boxcox_Se~inter, data = sub_BDD) # HomogÃ©nÃ©itÃ© des variances
...
# peut être causé par présence de NA, ce qui est le cas pour le mercure !!!
...
# Comment faire pour obtenir des résultats plus précis ?
require(dplyr)
...
####### Quel regroupement est il possible de faire au niveau des espèces ? #########
...
### Quelle différence entre %in% et == lorsque utilisation de subset ?
...
ggplot( filter(BDD, Groupe_station %in% "Chien_non_conta", Regime_principal %in% c("Carnivore", "Omnivore")), aes(x = d15N, y = conc_Hg_muscle_ppm, color = Genre, label = Code)) +
  ...
  geom_point(aes(shape=ordre)) +
  facet_grid(Regime_principal ~ ., scales = "free")
# équivalent à la sélection précédente.
...
df.sp <- filter(BDD, Regime_principal %in% "Carnivore") %.% # Sélection BDD
  group_by(Code) %.% # Sélection sp
  summarise(Hg_muscle_mean = mean(na.omit(conc_Hg_muscle_ppm)), d15N_mean = mean(na.omit(d15N)), Hg_muscle_se = se(na.omit(conc_Hg_muscle_ppm)), d15N_se = se(na.omit(d15N)))  # Sélection données à  calculer
...
df.sp <- filter(BDD, Regime_principal %in% "Omnivore") %.% # Sélection BDD
  group_by(Code) %.% # Sélection sp
  summarise(Hg_muscle_mean = mean(na.omit(conc_Hg_muscle_ppm)), d15N_mean = mean(na.omit(d15N)), Hg_muscle_se = se(na.omit(conc_Hg_muscle_ppm)), d15N_se = se(na.omit(d15N)))  # Sélection données à  calculer
#df.sp <- na.omit(merge(df.sp, select(filter(BDD, Regime_principal %in% "Carnivore"), ordre, Code), by = 'Code'))
...
df.sp <- BDD %.% # Sélection BDD
  group_by(Code) %.% # Sélection sp
  summarise(Hg_muscle_mean = mean(na.omit(conc_Hg_muscle_ppm)), d15N_mean = mean(na.omit(d15N)), Hg_muscle_se = se(na.omit(conc_Hg_muscle_ppm)), d15N_se = se(na.omit(d15N)), d13C_se = se(na.omit(d13C)), d13C_mean = mean(na.omit(d13C))) # Sélection données à  calculer
# Toujours la même question : comment faire pour appliquer "summarise" sur un facteur et ainsi ajouter l'information Regime_alimenaire pour chaque Code...
...
# Vue d'ensemble tous poissons selon régime
df.reg <- BDD %.% # Sélection BDD
  group_by(Regime_alter) %.% # Sélection sp
  summarise(Hg_muscle_mean = mean(na.omit(conc_Hg_muscle_ppm)), d15N_mean = mean(na.omit(d15N)), Hg_muscle_se = se(na.omit(conc_Hg_muscle_ppm)), d15N_se = se(na.omit(d15N)), d13C_se = se(na.omit(d13C)), d13C_mean = mean(na.omit(d13C)), Hg_foie_mean = mean(na.omit(conc_Hg_foie_ppm)), Hg_foie_se = se(na.omit(conc_Hg_foie_ppm)), Hg_branchie_mean = mean(na.omit(conc_Hg_branchie_ppm)), Hg_branchie_se = se(na.omit(conc_Hg_branchie_ppm))) # Sélection données à  calculer
...
pdf("2014_04_08_[Hg]_régime.pdf", width = 14, height = 9) # la fction pdf enregistre directement ds le dossier
print(pl1)
dev.off()
...
# Residuals vs Régimes
ggplot(BDD.res, aes(x = Regime_alter, Resid, colour = Regime_alter, shape = Groupe_station)) + geom_point() +
  xlab("Régimes") + ylab("Residuals")

# non respect des assumptions
# test avec transformations des données en boxcox et en log : toujours non respect. Besoin d'un test non param



# Test ANOVA uniquement avec les 3 régimes présents en abondance : Piscivores et Invertivores.
...
# comparaison des QQ plots. Problème
par(mfrow=c(2, 1))
...
# Toujours pas bon même après boxcox
...
# Toujours pas bon même après la transformation boxcox.

# Il faut réellement trouver un équivalent non paramétrique à une ANOVA factorielle
...
##### Siluriformes étaient plus contaminés en Se à Nouvelle-France #####
...
ggplot(BDD, aes(ls_mm, pds_g, color = Siluriformes)) +
  ...
  geom_point(alpha = 0.8) + # alternative à la ligne précédente
...
res.pca = PCA(scale.df, scale.unit=TRUE, ncp=5, graph=T) # Inutile car déjà argument scale dans la fonction ; même résultat obtenu dc rassurant
...
# calcul de différentes infos
df.Hg <- df %.% # Sélection BDD
  group_by(station) %.% # Sélection sp
  summarise(Hg_muscle_mean = mean(na.omit(conc_Hg_muscle_ppm)), Hg_muscle_se = se(na.omit(conc_Hg_muscle_ppm)), d15N_mean = mean(na.omit(d15N)), d15N_se = se(na.omit(d15N)), d13C_se = se(na.omit(d13C)), d13C_mean = mean(na.omit(d13C)), Hg_foie_mean = mean(na.omit(conc_Hg_foie_ppm)), Hg_foie_se = se(na.omit(conc_Hg_foie_ppm)), Hg_branchie_mean = mean(na.omit(conc_Hg_branchie_ppm)), Hg_branchie_se = se(na.omit(conc_Hg_branchie_ppm))) # Sélection données à  calculer
...
# plot sur données ne comprenant pas de NA pour [Hg]muscle
...
df.sansNA <- df[!(is.na(df$conc_Hg_muscle_ppm)), ] # même résultat que fonction précédente
...
p10 <- ggplot(df.sansNA, aes(x = station)) +
  geom_bar(aes(fill = Regime_alter), position = "fill") +
  scale_x_discrete(breaks = c(levels(df.sansNA$station)), labels=c(1:54)) # Remplace nom de la station par un numéro ; ordre alphabétique conservé
# répartition des régimes sur chaque station (sans prendre en compte le nb d'individus)
...
#### test non paramétrique sur différence de concentrations en Hg dans les muscles selon les stations

require(agricolae)

# detach("package:MASS", unload=TRUE)

kruskal.test(conc_Hg_muscle_ppm ~ station, data = df.sansNA) # Il esxiste des différences significatives


comparison <- kruskal(df.sansNA$conc_Hg_muscle_ppm, df.sansNA$station, alpha = 0.05, p.adj = "hochberg")

posthoc <- comparison[['groups']]
posthoc$trt <- gsub(" ","",posthoc$trt) # Tous les espaces après le nom doivent être supprimés pour pouvoir merge par la suite


BD <- select(df.sansNA, station, conc_Hg_muscle_ppm, Pression_anthro) # Subset plus simple à  manipuler
...
lettpos <- function(BD) boxplot(BD$conc_Hg_muscle_ppm, plot = FALSE)$stats[5,] # détermination d'un emplacement > à  la "moustache" du boxplot
test <- ddply(BD, .(station), lettpos) # Obtention de cette information pour chaque facteur (ici, Date)
test_f <- merge(test, posthoc, by.x = "station", by.y = "trt") # Les 2 tableaux sont réunis par rapport aux valeurs row.names
colnames(test_f)[2] <- "upper"
colnames(test_f)[4] <- "signif"
test_f$signif <- as.character(test_f$signif) # au cas où, pour que l'affichage se produise correctement. Pas forcément utile.
...
#### Répartition des concentrations selon régime alimentaire
...
BD <- select(df.omni, station, conc_Hg_muscle_ppm) # Subset plus simple à  manipuler
...
# PROBLEME avec test post hoc ; résultats aberrants
...
BD <- select(df.carni, station, conc_Hg_muscle_ppm) # Subset plus simple à  manipuler
...
casted <- dcast(df.sansNA, Genre ~ station) # dcast = opposé de la fonction melt
# Ne donne pas le résultat attendu. On obtient un tableau de contingence avec le nb d'individu de chq genre au niveau de chq station
# et non pas une liste d'espèce pour chaque station.

# Reduce(intersect, list(a, b , d)) # Du coup, ne fonctionne pas mais bonne synthaxe. Toujours utile au cas où

# d'après observation de casted, Moenkhausia semble être l'espèce la plus commune. Confirmé par ligne suivante
...
BD <- select(df.moenk, station, conc_Hg_muscle_ppm) # Subset plus simple à  manipuler
